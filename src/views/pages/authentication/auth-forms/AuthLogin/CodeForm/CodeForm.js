...
export function CodeForm({
    ...
    const { handleSubmit, values, setFieldValue, formatSeconds, isMobile, isDisabled } = useCodeForm(hackValue, onSubmit);
        ...
        <form onSubmit={handleSubmit}>
            <Box display="flex" flexDirection="column" height="100%">
                <Box flexGrow={1}>
                    ...
                    <Box mt={2} mb={2}>
                        <S.AutocompleteInput autoComplete="one-time-code" onChange={(e) => setFieldValue('code', e.target.value)} />
                        <CodeInput
                            size={4}
                            value={values.code}
                            error={!!errorMsg}
                    ...
                    <Box my={4}>
                        ...
                        {typeof resendTimer === 'number' && resendTimer && !maxAttempsAchieved ? (
                            <Box mt={4}>
                                <Typography>{`Отправить СМС еще раз можнео будет через ${formatSeconds(resendTimer)}`}</Typography>
                            </Box>
                        ) : null}
                    </Box>
                    {enableResend && !codeErrorMsg && !maxAttempsAchieved ? (
                        <Typography>
                            <TextButton sx={{ fontSize: 'inherit', fontFamily: 'inherit' }} disabled={loading} onClick={onResendCode}>
