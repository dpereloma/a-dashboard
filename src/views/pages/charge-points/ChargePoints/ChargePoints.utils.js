...
export const useChargePoints = () => {
    ...
    }, [searchValue, chargePoints, filterChargePoints]);

    const handleChange = (e) => setFilterValues((prev) => ({ ...prev, [e.target.name]: e.target.value }));
    ...
    useEffect(() => {
        const filledFilterParams = Object.entries(filterValues).filter(([k, v]) => v && v !== '-');
